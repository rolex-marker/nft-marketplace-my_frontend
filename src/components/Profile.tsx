...
interface ProfilePageProps {
  formFir : ProfileForm;
  reloadUserinfor: () => void;
}

  const ProfilePage: React.FC<ProfilePageProps> = ({formFir, reloadUserinfor}) => {
  const [form, setForm] = useState<ProfileForm>(formFir);
  ...
  const saveProfile = async () => {
    await axios.put(
      "https://83.234.227.67:4000/profile",
      form,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    <div className='create'>
      <div className="create-container">
        ...
        <form className='writeForm' autoComplete='off'>
          ...
          <div className="formGroup">
            <label>Bio</label>
            <textarea id="description" name="descripton"  onChange={e => setForm({ ...form, bio: e.target.value })} rows={4}
